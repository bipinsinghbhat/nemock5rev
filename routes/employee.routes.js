...
employeeRouter.post("/create",async(req,res)=>{
   ...
   employeeRouter.get("/", async (req, res) => {
    const { filter, sortBy, search, page } = req.query;
    const Page_Size = 5;
  
    try {
      let query = employeeModel.find();
  
      // Apply filter if provided
      if (filter) {
        query = query.where("department", filter);
      }
      ...
      const totalEmployees = await query.countDocuments();
      const totalPages = Math.ceil(totalEmployees / Page_Size);
